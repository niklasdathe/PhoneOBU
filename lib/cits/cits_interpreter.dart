...
abstract final class CitsInterpreter {
  static CitsMessage interpret(Map<String, Object?> json) {
    final messageSet =
        (json['messageSet'] ?? json['type'])?.toString().toUpperCase();
    final timestamp = DateTime.tryParse(
          json['sourceTimestamp']?.toString() ?? '',
        ) ??
        DateTime.now();
    final rawReference = json['rawFrameReference']?.toString() ??
    ...
    return switch (messageSet) {
      ...
      'IVIM' => IvimInformation(
          informationCode:
              json['informationCode']?.toString() ?? 'unknown',
          text: json['text']?.toString() ?? 'Infrastructure information',
          distanceMeters: _int(json['distanceM']),
          sourceTimestamp: timestamp,
          rawFrameReference: rawReference,
        ),
      _ => throw FormatException('Unsupported C-ITS message set: ' + messageSet.toString()),
    };
  }
